...
while (Number(howManyInARow) > Number(sizeBoardLENGTH) || Number(howManyInARow) > Number(sizeBoardWIDTH))
...
window.addEventListener("load", function () { changeBackground('lightslategray') });
function BuildBoard() {
	if (x == 0) {
		document.getElementById("ss").innerHTML = ("X turn").fontcolor("blue").fontsize(5);
		x = 1;
	}
		var num = 0;
		var textImgTag = "";
		...
		for (sh = 0; sh < sizeBoardLENGTH; sh++) {
			...
			for (am = 0; am < sizeBoardWIDTH; am++) {
				if (sizeBoardLENGTH > 12 || sizeBoardWIDTH > 12 || (sizeBoardLENGTH > 12 && sizeBoardWIDTH > 12)) {
					textImgTag = "<img src=" + images[ticTacToe[sh][am]] + " style='widows:20px; height:20px; width:20px' />";
					text = text + "<td id='" + num + "' onclick='SwitchPic(this);' style = width:20px; height:20px; width:20px >" + textImgTag + "</td>";
				}
				...
				else if (sizeBoardLENGTH > 6 || sizeBoardWIDTH > 6 || (sizeBoardLENGTH > 6 && sizeBoardWIDTH > 6)) {
				...
				else if (sizeBoardLENGTH > 5 || sizeBoardWIDTH > 5 || (sizeBoardLENGTH > 5 && sizeBoardWIDTH > 5)) {
					textImgTag = "<img src=" + images[ticTacToe[sh][am]] + " style='widows:60px; height:60px; width:60px' />";
					text = text + "<td id='" + num + "' onclick='SwitchPic(this);' style = width:60px; height:60px; width:60px >" + textImgTag + "</td>";
				}
				else if (sizeBoardLENGTH > 4 || sizeBoardWIDTH > 4 || (sizeBoardLENGTH > 4 && sizeBoardWIDTH > 4)){
					textImgTag = "<img src=" + images[ticTacToe[sh][am]] + " style='widows:80px; height:80px; width:80px' />";
					text = text + "<td id='" + num + "' onclick='SwitchPic(this);' style = width:80px; height:80px; width:80px >" + textImgTag + "</td>";
				}
				else{
					textImgTag = "<img src=" + images[ticTacToe[sh][am]] + " style='widows:100px; height:100px; width:100px' />";
					text = text + "<td id='" + num + "' onclick='SwitchPic(this);' style = width:100px; height:100px; width:100px >" + textImgTag + "</td>";
				}
				num++;
			}
			text = text + "</tr>";
...
function SwitchPic(currentThis) {
	...
	if (gameOver == false && turn == true && ticTacToe[CurrentRow][CurrentCol] == 0) {
		...
		turn = false;
		document.getElementById("ss").innerHTML = ("O turn").fontcolor("purple").fontsize(5);
	}
